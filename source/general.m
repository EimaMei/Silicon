...
NSView* NSView_initWithFrame(NSRect frameRect) {
    return [[[ViewClass alloc] initWithFrame:(frameRect)] autorelease];
}
...
NSTextField* NSTextField_initWithFrame(NSRect frameRect) {
    ...
    label = [[[NSTextField alloc] initWithFrame:(frameRect)] autorelease];
...
NSMenu* NSMenu_init(const char* title) {
    NSString* str = char_to_NSString(title);
    return [[[NSMenu alloc] initWithTitle:(str)] autorelease];
}
